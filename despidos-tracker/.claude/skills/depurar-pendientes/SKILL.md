...
4. **Escribí la entrada** siguiendo el paso 3 de `reescribir-titular` (título, bajada, moneda en letras, sector, geo). El sector tiene que ser uno de los que ya usa `data/curated_data.json` — a la fecha de este skill: Metalúrgica, Textil, Alimenticia, Calzado, Papel, Química, Agro, Transporte, Comercio, Ecommerce, Construcción, Medios, Seguros, Salud, Minería, Energía, Aeronáutica, Gastronomía, Entretenimiento, Otros. Si aparece un rubro nuevo que no encaja bien en ninguno, proponele al usuario sumar una categoría (como pasó con Salud/Ecommerce/Energía) en vez de forzarlo en "Otros" — y si la agrega, sumala también al array `SECTORES` de `admin/index.html` y a esta lista.
